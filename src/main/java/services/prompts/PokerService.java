...
public class PokerService extends PromptService{

    private static final PokerService instance = new PokerService();


    private PokerService() {
        super("Welcome to the poker table!");
    }
    ...
    private void play() {
        ...
        var takenNames = new ArrayList<>(CustomPokerPlayerDriver.getInstance().getAll().stream().map(CustomPokerPlayerDTO::name).toList());
        ...
        for(int i = 0; i < numPlayers; i++){
            ...
            AtomicReference<PokerPlayer> player = new AtomicReference<>();
            ActionPrompt.show(List.of("Bluffer", "Caller", "Random", "Custom"), List.of(
                    () -> {
                        var temp = new BlufferPokerPlayer(name);
                        temp.initFromConsole();
                        player.set(temp);
                    },
                    () -> {
                        var temp = new CallerPokerPlayer(name);
                        temp.initFromConsole();
                        player.set(temp);
                    },
                    () -> {
                        var temp = new RandomPokerPlayer(name);
                        temp.initFromConsole();
                        player.set(temp);
                    },
                    () -> {
                        var temp = new CustomPokerPlayer(name);
                        player.set(temp);
                    }
            ));
        ...
        game.addPlayer(new HumanPokerPlayer());
    ...
    private void editPlayers() {
        ...
        var customPokerPlayerDriver = CustomPokerPlayerDriver.getInstance();
        var availablePlayers = customPokerPlayerDriver.getAll();
        ...
        var actions = new ArrayList<>(availablePlayers.stream().map(
                playerDTO -> (Runnable) () -> {
                    ...
                    ActionPrompt.show(List.of("Edit", "Delete"), List.of(
                            () -> {
                                CustomPokerPlayer.update(playerDTO.name());
                            },
                            () -> {
                                customPokerPlayerDriver.delete(playerDTO.name());
                            }
                    ));
                }
        ).toList());
